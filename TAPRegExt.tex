...
Elements without a \xmlel{forMode} attribute apply to all modes for
which no more specific specification is given.  Elements with a
\xmlel{forMode} attribute override elements without one.  That is, to
find the applicable declaration for a particular access mode, clients
should first look for an element with the current access mode and then
for one without \xmlel{forMode}.  Whatever is found first is the
